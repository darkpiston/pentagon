...
3. Sign in to Azure and confirm your account can read the Key Vault secret:
   ...
   The same Google service account JSON is used for Vision and Gemini Enterprise. Ensure it has **Vertex AI User** (or equivalent Gemini Enterprise) permissions on the GCP project.
...
Uses the same request body as `ProcessImage`. Runs image analysis and returns a plain-language verification message.
...
```json
{
  "message": "It looks like you uploaded a photo of a flower. For verification, please upload a clear photo that shows both your face and your motorcycle."
}
```
...
├── Services/                           # Vision, Gemini, and Key Vault integration
